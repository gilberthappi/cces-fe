...
const AdminDashboard = () => {
	...
	const data = monthNames.map((month, index) => ({
		month,
		organizationCountByMonth: statisticCounts.organizationCountByMonth?.[index] || 0,
		citizenCountByMonth: statisticCounts.citizenCountByMonth?.[index] || 0,
			...
			<div className="flex-1 flex justify-between items-center">
					<div className="group flex flex-col items-center bg-white py-8 px-12 sm:py-12 sm:px-12 mr-2 lg:px-24 rounded-md hover:shadow-md transition-all duration-75 w-full flex-1 hover:bg-primary">
						<h1 className="font-bold text-xl sm:text-2xl text-gray-900 group-hover:text-white">
								{statisticCounts.organizationCountByMonth.reduce(
					...
					<div className="group flex flex-col items-center bg-white py-8 px-12 sm:py-12 sm:px-12 mr-2 lg:px-24 rounded-md hover:shadow-md transition-all duration-75 w-full flex-1 hover:bg-primary">
						<h1 className="font-bold text-xl sm:text-2xl text-gray-900 group-hover:text-white">
								{statisticCounts.citizenCountByMonth.reduce(
										(a: number, b: number) => (Number(a) || 0) + (Number(b) || 0),
										0,
									) || 0}
						</h1>
			...
			<div className="bg-white rounded-md p-4 shadow-md flex-1">
				...
				<ResponsiveContainer width="100%" height={300}>
					<LineChart data={data}>
						...
						<Tooltip />
						<Legend />
						<Line type="monotone" dataKey="organizationCountByMonth" stroke="#82ca9d" name="Organization" />
					</LineChart>
				</ResponsiveContainer>
			</div>
			<div className="bg-white rounded-md p-4 shadow-md flex-1">
				...
				<ResponsiveContainer width="100%" height={300}>
					<BarChart data={data}>
						...
						<Tooltip />
						<Legend />
						<Bar dataKey="feedbackCountByMonth" fill="#cdbd8e" name="Complaints" />
						<Bar dataKey="citizenCountByMonth" fill="RGB(138, 144, 154)" name="Citizen" />
					</BarChart>
				</ResponsiveContainer>
			</div>
		</div>
